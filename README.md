...
1. verifies and installs plain Hugo 0.124.1;
2. runs source, workflow-security, style, template, and JavaScript checks;
3. builds the self-contained Hugo site;
4. validates generated metadata, landmarks, images, local links, fragments, and external records;
5. runs Pa11y at desktop and mobile widths;
6. runs mobile and desktop Lighthouse audits;
7. uploads successful PR evidence or failure diagnostics and deploys only after all gates pass;
8. verifies the deployed release; and
9. sends a non-blocking IndexNow notification.

The two-page CV is validated in a separate path-filtered workflow so normal site deployments do not install a full TeX distribution.
...
# In another terminal, with CHROME_PATH set:
python3 scripts/check_responsive.py --base-url http://127.0.0.1:4173/ --chrome "$CHROME_PATH"
PA11Y_WIDTH=1440 PA11Y_HEIGHT=1000 npx pa11y-ci --config .pa11yci.cjs
LHCI_FORM_FACTOR=desktop npx lhci autorun --config=.lighthouserc.cjs
```
...
See [`PREDEPLOY-READINESS.md`](PREDEPLOY-READINESS.md) for the release gates, corrected CI failure, and post-deployment ownership steps.
