...
for workflow in workflows:
    ...
    for index, line in enumerate(lines):
        ...
        if ref.startswith('actions/checkout@') and workflow.name != 'sync-publications.yaml':
    ...
    if workflow.name != 'sync-publications.yaml' and re.search(r'^\s*contents:\s*write\s*$', text, re.MULTILINE):
