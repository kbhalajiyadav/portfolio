...
3. The external-link checker treated absolute `bhalaji.com` links as third-party URLs, so the new `/research/` route was tested against the previous live deployment and returned 404 even though `public/research/index.html` existed.

The identity photograph now has a durable non-empty alternative, remains visually and assistively hidden, and is checked by source regression tests. The research link now targets the exact generated heading ID and the stale target is blocked from returning. Absolute same-site URLs are now resolved by `check_site.py` against the freshly generated public tree and excluded from the network checker; only genuinely third-party URLs are contacted.
...
8. External-link validation checks only third-party records, fails confirmed HTTP 404/410 responses, and reports provider bot blocks or transient failures separately. Absolute bhalaji.com links are resolved and validated against the freshly generated public tree; only third-party domains are contacted by the network checker.
...
- Focused regressions passed for absolute same-site resolution and exclusion from third-party network checks.
