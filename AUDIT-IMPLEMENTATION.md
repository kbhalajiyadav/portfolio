...
- The CV source is regenerated during the site build, while PDF compilation is isolated to a path-filtered CV workflow.
...
- Corrected the minified IndieWeb-photo alt failure and the exact research-page fragment target.
- Removed the obsolete HugoBlox/Go module layer; the site is now plain Hugo with built-in HTML/RSS outputs.
- Split expensive TeX CV validation from normal site deployment.
- Added immutable action SHA pins, verified Hugo checksums, exact direct npm audit-tool versions, same-site-aware external-link checks, Pa11y, mobile/desktop Lighthouse, failure artifacts, and a post-deployment smoke test.
- Added workflow-security, ambiguous-link-text, landmark, and skip-link-target validation.
